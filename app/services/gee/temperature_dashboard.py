# app/services/gee/temperature_dashboard.py
import ee
...
def get_current_season_overview(
    ...
    return {
        ...
        "heat_stress_days_above_35C": hot_days_count.getInfo()
    }


# 30 YEAR CLIMATE ANOMALY
def get_seasonal_anomaly_30yr(
    geometry: ee.Geometry,
    season: str,
    year: int
) -> dict:
    ...
    for y in climatology_years:
        climatology_collection = climatology_collection.merge(
            _season_collection(y, season)
        )

    climatology_mean = climatology_collection.mean()
...
# 10 YEAR TREND
def get_10yr_temperature_trend(
    geometry: ee.Geometry,
    season: str,
    year: int
) -> dict:
    ...
    # Simple warming check
    warming = values[-1]["mean_temp_c"] > values[0]["mean_temp_c"]
...
def get_agronomic_interpretation(mean_temp_c: float, heat_days: int) -> dict:
