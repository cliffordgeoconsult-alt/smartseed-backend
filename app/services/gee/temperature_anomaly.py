# app/services/gee/temperature_anomaly.py
# app/services/gee/temperature_anomaly.py
import ee

ERA5_DAILY = "ECMWF/ERA5_LAND/DAILY_AGGR"

SEASONS = {
    "MAM": [3, 4, 5],
    "JJA": [6, 7, 8],
    "SON": [9, 10, 11],
    "DJF": [12, 1, 2]
}


def _season_collection(year: int, season: str) -> ee.ImageCollection:
    months = SEASONS[season]
    collection = ee.ImageCollection([])

    for m in months:

        # DJF handling
        if season == "DJF" and m in [1, 2]:
            y = year + 1
        else:
            y = year

        start = ee.Date.fromYMD(y, m, 1)
        end = start.advance(1, "month")

        col = (
            ee.ImageCollection(ERA5_DAILY)
            .filterDate(start, end)
            .select("temperature_2m")
        )

        collection = collection.merge(col)

    return collection


def get_temperature_seasonal_anomaly(
    geometry: ee.Geometry,
    season: str,
    year: int
) -> dict:

    if season not in SEASONS:
        raise ValueError("Invalid season. Use MAM, JJA, SON, DJF")

    target_mean = _season_collection(year, season).mean()
    ref_mean = _season_collection(year - 1, season).mean()

    target_stats = target_mean.reduceRegion(
        reducer=ee.Reducer.mean(),
        geometry=geometry,
        scale=1000,
        bestEffort=True,
        maxPixels=1e9
    )

    ref_stats = ref_mean.reduceRegion(
        reducer=ee.Reducer.mean(),
        geometry=geometry,
        scale=1000,
        bestEffort=True,
        maxPixels=1e9
    )

    target_k = ee.Number(target_stats.get("temperature_2m"))
    ref_k = ee.Number(ref_stats.get("temperature_2m"))

    target_c = target_k.subtract(273.15)
    ref_c = ref_k.subtract(273.15)
    anomaly = target_c.subtract(ref_c)

    return {
        "season": season,
        "target_year": year,
        "reference_year": year - 1,
        "mean_temp_target_c": target_c.getInfo(),
        "mean_temp_reference_c": ref_c.getInfo(),
        "mean_temp_anomaly_c": anomaly.getInfo()
    }